...
local topicList = {
	...
	CHANGE_AMBER_CRYSTAL = 18,
	CHANGE_AMBER_CRYSTAL_CONSENT = 19,
	CHANGE_DEMONIAC_AMBER = 20,
	CHANGE_DEMONIAC_AMBER_CONSENT = 21
}
...
local function creatureSayCallback(cid, type, msg)
	...
	elseif msgcontains(msg, "balance") then
		...
		elseif player:getBankBalance() >= 100000 then
			...
			return true
		elseif player:getBankBalance() >= 10000000 then
			npcHandler:say("You certainly one of the most rich guys in the game. Your account balance is " .. player:getBankBalance() .. " gold.", cid)
			return true
		elseif player:getBankBalance() >= 1000000000 then
			npcHandler:say("Insane balance!. Your account balance is " .. player:getBankBalance() .. " gold.", cid)
			return true
		end
		npcHandler:say("Your account balance is " .. player:getBankBalance() .. " gold.", cid)
		return true
	elseif msgcontains(msg, "deposit") then
		count[cid] = getTrueOnHand and getTrueOnHand(player) or player:getMoney()
		if count[cid] < 1 then
	...
	elseif npcHandler.topic[cid] == topicList.DEPOSIT_CONSENT then
		if msgcontains(msg, "yes") then
			local deposited = depositAllOnHandToBank(player)
			if deposited > 0 then
			npcHandler:say(
				"Alright, we have added the amount of " .. deposited ..
		...
		if msgcontains(msg, "yes") then
			if player:canCarryMoney(count[cid]) then
				if not player:withdrawMoney(count[cid]) then
					print("player withdraw", player:withdrawMoney(count[cid]))
					npcHandler:say("There is not enough gold in your account.", cid)
				else
	...
	elseif msgcontains(msg, "change") then
		...
		if #parts > 1 then
			...
			elseif parts[2]:lower() == "amber" then
				npcHandler:say("How many amber coins would you like to change into crystals?", cid)
				npcHandler.topic[cid] = topicList.CHANGE_AMBER_CRYSTAL
			elseif parts[2]:lower() == "demoniac" then
				npcHandler:say("How many demoniac coins would you like to change into amber?", cid)
				npcHandler.topic[cid] = topicList.CHANGE_DEMONIAC_AMBER
			end
		else
			npcHandler:say("Would you like to change {gold}, {platinum}, {crystal}, {amber} or {demoniac} coins?", cid)
	...
	elseif msgcontains(msg, "crystal") and npcHandler.topic[cid] == topicList.CHANGE_GOLD_CHOOSE then
		npcHandler:say("How many crystal coins would you like to change into platinum?", cid)
		npcHandler.topic[cid] = topicList.CHANGE_CRYSTAL_PLATINUM
	elseif msgcontains(msg, "amber") and npcHandler.topic[cid] == topicList.CHANGE_GOLD_CHOOSE then
			npcHandler:say("How many amber coins would you like to change into platinum?", cid)
			npcHandler.topic[cid] = topicList.CHANGE_AMBER_CRYSTAL
	elseif msgcontains(msg, "demoniac") and npcHandler.topic[cid] == topicList.CHANGE_GOLD_CHOOSE then
				npcHandler:say("How many demoniac coins would you like to change into platinum?", cid)
				npcHandler.topic[cid] = topicList.CHANGE_DEMONIAC_AMBER
	...
	elseif npcHandler.topic[cid] == topicList.CHANGE_AMBER_CRYSTAL then
		if getMoneyCount(msg) < 1 then
			npcHandler:say("Sorry, you do not have enough amber coins.", cid)
			npcHandler.topic[cid] = topicList.NONE
		else
			count[cid] = getMoneyCount(msg)
			npcHandler:say("So you would like me to change " .. count[cid] .. " of your amber coins into " .. count[cid] * 100 .. " crystal coins for you?", cid)
			npcHandler.topic[cid] = topicList.CHANGE_AMBER_CRYSTAL_CONSENT
		end
	elseif npcHandler.topic[cid] == topicList.CHANGE_DEMONIAC_AMBER then
		if getMoneyCount(msg) < 1 then
			npcHandler:say("Sorry, you do not have enough demoniac coins.", cid)
			npcHandler.topic[cid] = topicList.NONE
		else
			count[cid] = getMoneyCount(msg)
			npcHandler:say("So you would like me to change " .. count[cid] .. " of your demoniac coins into " .. count[cid] * 100 .. " amber coins for you?", cid)
			npcHandler.topic[cid] = topicList.CHANGE_DEMONIAC_AMBER_CONSENT
		end
	elseif npcHandler.topic[cid] == topicList.CHANGE_CRYSTAL_PLATINUM_CONSENT then
		...
		npcHandler.topic[cid] = topicList.NONE
	elseif npcHandler.topic[cid] == topicList.CHANGE_AMBER_CRYSTAL_CONSENT then
		if msgcontains(msg, "yes") then
			if player:getFreeCapacity() >= getMoneyWeight(count[cid]) then
				if player:removeItem(ITEM_AMBER_COIN, count[cid]) then
					player:addItem(ITEM_CRYSTAL_COIN, count[cid] * 100)
					npcHandler:say("Here you are.", cid)
				else
					npcHandler:say("Sorry, you do not have enough amber coins.", cid)
				end
		...
		npcHandler.topic[cid] = topicList.NONE
	elseif npcHandler.topic[cid] == topicList.CHANGE_DEMONIAC_AMBER_CONSENT then
		if msgcontains(msg, "yes") then
			if player:getFreeCapacity() >= getMoneyWeight(count[cid]) then
				if player:removeItem(ITEM_DEMONIAC_COIN, count[cid]) then
					player:addItem(ITEM_AMBER_COIN, count[cid] * 100)
					npcHandler:say("Here you are.", cid)
				else
					npcHandler:say("Sorry, you do not have enough demoniac coins.", cid)
				end
