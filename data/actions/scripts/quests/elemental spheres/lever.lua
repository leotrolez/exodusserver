local config = {
	...
	{position = Position(33268, 31833, 10), itemid = 8304, toPosition = Position(33268, 31833, 12), vocationId = {1, 9}} -- ILLUSIONIST
}
...
function onUse(player, item, fromPosition, target, toPosition, isHotkey)
	...
	for i = 1, #config do
		...
		if type(config[i].vocationId) == "table" then
			for _, vId in ipairs(config[i].vocationId) do
				if vocationId == vId then
					validVocation = true
					break
				end
			end
		else
			validVocation = vocationId == config[i].vocationId
		end

		if not validVocation or creature:getItemCount(config[i].itemid) < 1 or (creature:getStorageValue(Storage.ElementalSphere.QuestLine) or -1) < 1 then
			player:say('You need one player of each vocation having completed the Elemental Spheres quest and also carrying the elemental rare item.', TALKTYPE_MONSTER_SAY, false, 0, Position(33268, 31835, 10))
			return true
		end
