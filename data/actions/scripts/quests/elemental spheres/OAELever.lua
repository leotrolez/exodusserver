...
function onUse(player, item, fromPosition, target, toPosition, isHotkey)
	...
	local function getMatchingPositionIndex(positions, comparePos)
	for i = 1, #positions do
		if positions[i].x == comparePos.x and positions[i].y == comparePos.y and positions[i].z == comparePos.z then
			return i
		end
	end
	return nil
	end
	local index = getMatchingPositionIndex(config.positions, player:getPosition())
	if not index then
		player:say("You are not standing on a valid activation tile.", TALKTYPE_MONSTER_SAY)
		return true
	end
	...
	if not walls then
		player:say("Wall config missing for this lever.", TALKTYPE_MONSTER_SAY)
		return true
	end
