...
function PlayerSetStorageValue(cid, key, value) local p = Player(cid) return p and p:setStorageValue(key, value) or false end
function PlayerGetStorageValue(cid, key, value) local p = Player(cid) return p and p:getStorageValue(key, value) end
function PlayerSetBalance(cid, balance) local p = Player(cid) return p and p:setBankBalance(balance) or false end
function PlayerAddMoney(cid, money) local p = Player(cid) return p and p:addMoney(money) or false end
function PlayerRemoveMoney(cid, money) local p = Player(cid) return p and p:removeMoney(money) or false end
function PlayerAddSoul(cid, soul) local p = Player(cid) return p and p:addSoul(soul) or false end
function PlayerSetVocation(cid, vocation) local p = Player(cid) return p and p:setVocation(Vocation(vocation)) or false end
function PlayerSetTown(cid, town) local p = Player(cid) return p and p:setTown(Town(town)) or false end
function PlayerSetGroupId(cid, groupId) local p = Player(cid) return p and p:setGroup(Group(groupId)) or false end
function PlayerSetSex(cid, sex) local p = Player(cid) return p and p:setSex(sex) or false end
function PlayerSetGuildLevel(cid, level) local p = Player(cid) return p and p:setGuildLevel(level) or false end
function PlayerSetGuildNick(cid, nick) local p = Player(cid) return p and p:setGuildNick(nick) or false end
function PlayerSetOfflineTrainingSkill(cid, skillId) local p = Player(cid) return p and p:setOfflineTrainingSkill(skillId) or false end
function ShowTextDialog(cid, itemId, text) local p = Player(cid) return p and p:showTextDialog(itemId, text) or false end
function PlayerAddItemEx(cid, uid, ...) local p = Player(cid) return p and p:addItemEx(Item(uid), ...) or false end
function PlayerRemoveItem(cid, itemid, count, ...) local p = Player(cid) return p and p:removeItem(itemid, count, ...) or false end
function PlayerAddPremiumDays(cid, days) local p = Player(cid) return p and p:addPremiumDays(days) or false end
function PlayerRemovePremiumDays(cid, days) local p = Player(cid) return p and p:removePremiumDays(days) or false end
function PlayerAddBlessing(cid, blessing) local p = Player(cid) return p and p:addBlessing(blessing) or false end
function PlayerAddOutfit(cid, lookType, addons) local p = Player(cid) return p and p:addOutfitAddon(lookType, addons) or false end
function PlayerRemoveOutfit(cid, lookType, addons)
	local player = Player(cid)
	if player == nil then
		return false
	end
...
function PlayerWearOutfit(cid, lookType, addons) local p = Player(cid) return p and p:hasOutfit(lookType, addons) or false end
function PlayerAddMount(cid, mountId) local p = Player(cid) return p and p:addMount(mountId) or false end
function PlayerRemoveMount(cid, mountId) local p = Player(cid) return p and p:removeMount(mountId) or false end
function PlayerSendCancel(cid, text) local p = Player(cid) return p and p:sendCancelMessage(text) or false end
function PlayerFeed(cid, food) local p = Player(cid) return p and p:feed(food) or false end
function PlayerLearnInstantSpell(cid, name) local p = Player(cid) return p and p:learnSpell(name) or false end
function PlayerPopupFYI(cid, message) local p = Player(cid) return p and p:popupFYI(message) or false end
function SendTutorial(cid, tutorialId) local p = Player(cid) return p and p:sendTutorial(tutorialId) or false end
function AddMapMark(cid, pos, type, description) local p = Player(cid) return p and p:addMapMark(pos, type, description or "") or false end
function PlayerSendTextMessage(cid, type, text, ...) local p = Player(cid) return p and p:sendTextMessage(type, text, ...) or false end
function SendAnimatedText() debugPrint("Deprecated function.") return true end
function PlayerAddExp(cid, exp, useMult, ...)
	local player = Player(cid)
	if player == nil then
		return false
	end
...
function PlayerAddManaSpent(cid, mana) local p = Player(cid) return p and p:addManaSpent(mana) or false end
function PlayerAddSkillTry(cid, skillid, n) local p = Player(cid) return p and p:addSkillTries(skillid, n) or false end
function PlayerAddMana(cid, mana, ...) local p = Player(cid) return p and p:addMana(mana, ...) or false end
function PlayerJoinParty(cid, leaderId)
	local player = Player(cid)
	if player == nil then
		return false
	end
...
function SendMagicEffect(pos, magicEffect, ...) return Position(pos):sendMagicEffect(magicEffect, ...) end
function SendDistanceShoot(fromPos, toPos, distanceEffect, ...) return Position(fromPos):sendDistanceEffect(toPos, distanceEffect, ...) end
...
function PlayerTakeItem(cid, itemid, count)
