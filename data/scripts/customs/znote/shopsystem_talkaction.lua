...
function talkaction.onSay(player)
	...
	if player:getStorageValue(storage) <= os.time() then
		...
		-- Create the query
		local orderQuery = db.storeQuery("SELECT `id`, `type`, `itemid`, `mountid`, `count` FROM `znote_shop_orders` WHERE `account_id` = " .. player:getAccountId() .. ";")
		local served = false
			...
			repeat
				...
				if q_type == 5 then
					...
					if itemid > 10000 then
						local first = math.floor(itemid/10000)
						table.insert(outfits, first)
						itemid = itemid - (first * 10000)
					end
				...
				if Game.getClientVersion().min >= 870 then
					local q_mountid = result.getNumber(orderQuery, "mountid")
					if q_type == 6 then
						served = true
						-- Make sure player don't already have this outfit and addon
						if not player:hasMount(q_mountid) then
							db.query("DELETE FROM `znote_shop_orders` WHERE `id`="..q_id..";")
