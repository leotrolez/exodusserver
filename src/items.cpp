...
const std::unordered_map<std::string, ItemParseAttributes_t> ItemParseAttributesMap = {
	...
	{"wrapid", ITEM_PARSE_WRAPID},
	{"unwrapid", ITEM_PARSE_UNWRAPID},
	...
	{"skillherbalist", ITEM_PARSE_SKILLHERBALIST},
	{"skillarmorsmith", ITEM_PARSE_SKILLARMORSMITH},
	{"skillweaponsmith", ITEM_PARSE_SKILLWEAPONSMITH},
	{"skilljewelsmith", ITEM_PARSE_SKILLJEWELSMITH},
	...
	{"suppresssplashed", ITEM_PARSE_SUPPRESSSPLASHED},
	{"suppressbewitched", ITEM_PARSE_SUPPRESSBEWITCHED},
...
void Items::buildInventoryList()
{
	...
	for (const auto& type: items) {
		if (type.weaponType != WEAPON_NONE || type.ammoType != AMMO_NONE ||
			...
			type.slotPosition & SLOTP_DECK ||
			type.slotPosition & SLOTP_BADGE ||
			type.slotPosition & SLOTP_SHIP ||
			type.slotPosition & SLOTP_BELT ||
			type.slotPosition & SLOTP_GLOVES)
		{
	...
	for (auto attributeNode : itemNode.children()) {
		...
		if (parseAttribute != ItemParseAttributesMap.end()) {
			...
			switch (parseType) {
				...
				case ITEM_PARSE_WRAPID: {
					it.wrapId = pugi::cast<uint16_t>(valueAttribute.value());
					break;
				}

				case ITEM_PARSE_UNWRAPID: {
					it.unwrapId = pugi::cast<uint16_t>(valueAttribute.value());
					break;
				}
				...
				case ITEM_PARSE_SLOTTYPE: {
					...
					} else if (tmpStrValue == "deckbadge") {
						it.slotPosition |= SLOTP_DECKBAD;
					} else if (tmpStrValue == "ship") {
						it.slotPosition |= SLOTP_SHIP;
					} else if (tmpStrValue == "beltgloves") {
						it.slotPosition |= SLOTP_BELTGLOVES;
					} else {
				...
				case ITEM_PARSE_SKILLHERBALIST: {
					abilities.skills[SKILL_HERBALIST] = pugi::cast<int32_t>(valueAttribute.value());
					break;
				}

				case ITEM_PARSE_SKILLARMORSMITH: {
					abilities.skills[SKILL_ARMORSMITH] = pugi::cast<int32_t>(valueAttribute.value());
					break;
				}

				case ITEM_PARSE_SKILLWEAPONSMITH: {
					abilities.skills[SKILL_WEAPONSMITH] = pugi::cast<int32_t>(valueAttribute.value());
					break;
				}
				case ITEM_PARSE_SKILLJEWELSMITH: {
					abilities.skills[SKILL_JEWELSMITH] = pugi::cast<int32_t>(valueAttribute.value());
					break;
				}
				...
				case ITEM_PARSE_SUPPRESSSPLASHED: {
					if (valueAttribute.as_bool()) {
						abilities.conditionSuppressions |= CONDITION_SPLASHED;
					}
					break;
				}

				case ITEM_PARSE_SUPPRESSBEWITCHED: {
					if (valueAttribute.as_bool()) {
						abilities.conditionSuppressions |= CONDITION_BEWITCHED;
					}
					break;
				}
				...
				case ITEM_PARSE_FIELD: {
					...
					if (tmpStrValue == "fire") {
						...
						conditionDamage = new ConditionDamage(CONDITIONID_COMBAT, CONDITION_BEWITCHED);
						combatType = COMBAT_ARCANEDAMAGE;
					} else if (tmpStrValue == "water") {
						conditionDamage = new ConditionDamage(CONDITIONID_COMBAT, CONDITION_SPLASHED);
				...
				case ITEM_PARSE_ELEMENTWATER: {
					abilities.elementDamage = pugi::cast<uint16_t>(valueAttribute.value());
					abilities.elementType = COMBAT_WATERDAMAGE;
					break;
				}
