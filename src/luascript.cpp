...
void LuaScriptInterface::registerFunctions()
{
	...
	registerEnum(CONDITION_PARAM_SKILL_HERBALIST)
	registerEnum(CONDITION_PARAM_SKILL_ARMORSMITH)
	registerEnum(CONDITION_PARAM_SKILL_WEAPONSMITH)
	registerEnum(CONDITION_PARAM_SKILL_JEWELSMITH)
	...
	registerEnum(CONDITION_PARAM_SKILL_HERBALISTPERCENT)
	registerEnum(CONDITION_PARAM_SKILL_ARMORSMITHPERCENT)
	registerEnum(CONDITION_PARAM_SKILL_WEAPONSMITHPERCENT)
	registerEnum(CONDITION_PARAM_SKILL_JEWELSMITHPERCENT)
	...
	registerEnum(CONST_SLOT_DECK)
	registerEnum(CONST_SLOT_BADGE)
	registerEnum(CONST_SLOT_SHIP)
	registerEnum(CONST_SLOT_BELT)
	registerEnum(CONST_SLOT_GLOVES)
	...
	registerEnum(ITEM_ATTRIBUTE_UNWRAPID)
	...
	registerEnum(ITEM_AMBER_COIN)
	registerEnum(ITEM_DEMONIAC_COIN)
	...
	registerEnum(SKILL_ARMORSMITH)
	registerEnum(SKILL_WEAPONSMITH)
	registerEnum(SKILL_JEWELSMITH)
	registerEnum(SKILL_HERBALIST)
	...
	registerEnum(GUILDEMBLEM_CUSTOM);
	...
	registerEnum(SLOTP_DECK)
	registerEnum(SLOTP_BADGE)
	registerEnum(SLOTP_SHIP)
	registerEnum(SLOTP_BELT)
	registerEnum(SLOTP_GLOVES)
	...
	registerEnum(SPEECHBUBBLE_BOAT);
	registerEnum(SPEECHBUBBLE_MISSION);
	registerEnum(SPEECHBUBBLE_EXPEDITION);
	registerEnum(SPEECHBUBBLE_DORN_REP);
	registerEnum(SPEECHBUBBLE_FROST_REP);
	registerEnum(SPEECHBUBBLE_HALLOW_REP);
	registerEnum(SPEECHBUBBLE_VARDEN_REP);
	registerEnum(SPEECHBUBBLE_VYRSK_REP);
	...
	registerEnum(MAPMARK_QUEST);
	registerEnum(MAPMARK_GOLD);
	registerEnum(MAPMARK_EXPEDITION);
	registerEnum(MAPMARK_DUNGEON);
	registerEnum(MAPMARK_CRAFT);
	...
	registerMethod("Player", "removeWings", LuaScriptInterface::luaPlayerRemoveWing);
	...
	registerMethod("ItemType", "getItemTypeAbsorbPercent", LuaScriptInterface::luaItemTypeGetAbsorbPercent);
	...
	registerMethod("MonsterType", "minLevel", LuaScriptInterface::luaMonsterTypeMinLevel);
	registerMethod("MonsterType", "maxLevel", LuaScriptInterface::luaMonsterTypeMaxLevel);
...
int LuaScriptInterface::luaItemGetAttribute(lua_State* L)
{
	...
	if (isNumber(L, 2)) {
		...
		if (key == "wrapid") {
			const ItemType& it = Item::items[item->getID()];
			lua_pushnumber(L, it.wrapId);
			return 1;
		} else if (key == "unwrapid") {
		if (item->hasAttribute(ITEM_ATTRIBUTE_UNWRAPID)) {
			lua_pushnumber(L, item->getIntAttr(ITEM_ATTRIBUTE_UNWRAPID));
		} else {
			const ItemType& it = Item::items[item->getID()];
			lua_pushnumber(L, it.unwrapId);
		}
			return 1;
		}

		attribute = stringToItemAttribute(key);
	} else {
		attribute = ITEM_ATTRIBUTE_NONE;
	}
...
{
    // creature:addHealth(damageType as string, healthChange)
	// example: creature:addHealth("energy", -500) if is positive then creature:addHealth(100) no string needed
    Creature* creature = getUserdata<Creature>(L, 1);
    if (!creature) {
        lua_pushnil(L);
        return 1;
    ...
    std::string damageType = getString(L, 2);
    int32_t healthChange = getNumber<int32_t>(L, 3);
    ...
    if (damageType == "fire") {
        ...
        damage.primary.type = COMBAT_EARTHDAMAGE;
    } else if (damageType == "water") {
        damage.primary.type = COMBAT_WATERDAMAGE;
    } else if (damageType == "arcane") {
        damage.primary.type = COMBAT_ARCANEDAMAGE;
    } else if (damageType == "death") {
        damage.primary.type = COMBAT_DEATHDAMAGE;
    } else if (damageType == "ice") {
        damage.primary.type = COMBAT_ICEDAMAGE;
    } else if (damageType == "holy") {
        damage.primary.type = COMBAT_HOLYDAMAGE;
    } else if (damageType == "lifedrain") {
        damage.primary.type = COMBAT_LIFEDRAIN;
    } else if (damageType == "manadrain") {
        damage.primary.type = COMBAT_MANADRAIN;
    } else if (damageType == "drown") {
        damage.primary.type = COMBAT_DROWNDAMAGE;
...
int LuaScriptInterface::luaPlayerRemoveWing(lua_State* L) {
    ...
    pushBoolean(L, player->removeWings(wingId));
    return 1;
}
...
int LuaScriptInterface::luaItemTypeGetAbsorbPercent(lua_State* L) {
	// Stack: [itemTypeId, combatType]
	uint16_t itemTypeId = getNumber<uint16_t>(L, 1); // Use the first argument (itemTypeId)
	CombatType_t combatType = static_cast<CombatType_t>(getNumber<int>(L, 2)); // Use the second argument (combatType)
	const ItemType& itemType = Item::items[itemTypeId];
	// Ensure abilities exist
	if (!itemType.abilities) {
		lua_pushnil(L);
		return 1; // Return nil if no abilities
	}
	// Convert combatType to index
	int16_t index = combatTypeToIndex(combatType);
	if (index < 0 || index >= COMBAT_COUNT) {
		lua_pushnil(L); // Invalid combat type
		return 1;
	}
	// Push the absorb percent to the Lua stack
	lua_pushnumber(L, itemType.abilities->absorbPercent[index]);
...
int LuaScriptInterface::luaMonsterTypeMinLevel(lua_State* L) {
	// get: monsterType:minLevel()
	MonsterType* monsterType = getUserdata<MonsterType>(L, 1);
	if (monsterType) {
		if (lua_gettop(L) == 1) {
...
int LuaScriptInterface::luaMonsterTypeMaxLevel(lua_State* L) {
	// get: monsterType:maxLevel()
	MonsterType* monsterType = getUserdata<MonsterType>(L, 1);
	if (monsterType) {
		if (lua_gettop(L) == 1) {
			lua_pushnumber(L, monsterType->info.maxLevel);
		} else {
			monsterType->info.maxLevel = getNumber<int32_t>(L, 2);
			pushBoolean(L, true);
		}
	} else {
		lua_pushnil(L);
	...
	if (monsterType) {
		if (lua_gettop(L) == 1) {
			...
			if (immunity == "physical") {
				...
				monsterType->info.damageImmunities |= COMBAT_ARCANEDAMAGE;
				pushBoolean(L, true);
	...
	if (monsterType) {
		if (lua_gettop(L) == 1) {
			...
			if (immunity == "physical") {
				...
				monsterType->info.conditionImmunities |= CONDITION_SPLASHED;
				pushBoolean(L, true);
			} else if (immunity == "arcane") {
				monsterType->info.conditionImmunities |= CONDITION_BEWITCHED;
				pushBoolean(L, true);
...
int LuaScriptInterface::luaMoveEventSlot(lua_State* L)
{
	...
	if (moveevent->getEventType() == MOVE_EVENT_EQUIP || moveevent->getEventType() == MOVE_EVENT_DEEQUIP) {
		...
		} else if (slotName == "deckbadge") {
			moveevent->setSlot(SLOTP_DECKBAD);
		} else if (slotName == "ship") {
			moveevent->setSlot(SLOTP_SHIP);
		} else if (slotName == "beltgloves") {
			moveevent->setSlot(SLOTP_BELTGLOVES);
		} else {
	...
	if (weapon) {
		if (!getNumber<CombatType_t>(L, 2)) {
			...
			if (tmpStrValue == "earth") {
				...
				weapon->params.combatType = COMBAT_WATERDAMAGE;
			} else if (tmpStrValue == "arcane") {
				weapon->params.combatType = COMBAT_ARCANEDAMAGE;
	...
	if (weapon) {
		...
		if (!getNumber<CombatType_t>(L, 3)) {
			...
			if (tmpStrValue == "earth") {
				...
				it.abilities.get()->elementType = COMBAT_WATERDAMAGE;
			} else if (tmpStrValue == "arcane") {
