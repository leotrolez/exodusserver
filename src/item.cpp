...
bool Item::applyRarityEffects(Item* item, int rarityId) {
    if (!item) return false;
    if (rarityId <= 0) return false;
...
Item* Item::CreateItemWithRarity(uint16_t type, uint16_t count, int rarityId) {
    Item* newItem = CreateItem(type, count);
    if (!newItem || rarityId <= 0) return newItem;

    if (Item::applyRarityEffects(newItem, rarityId)) {
...
Item* Item::CreateItem(const uint16_t type, uint16_t count /*= 0*/)
{
	...
	if (it.id != 0) {
		...
		if (newItem) {
		if (const auto attr = newItem->getCustomAttribute("rarity")) {
			int rarityId = 0;
			const auto& v = attr->value;
			if (v.type() == typeid(int64_t)) rarityId = boost::get<int64_t>(v);
			if (rarityId > 0) Item::applyRarityEffects(newItem, rarityId);
		}
		}
...
Attr_ReadValue Item::readAttr(AttrTypes_t attr, PropStream& propStream)
{
	switch (attr) {
		...
		case ATTR_UNWRAPID: {
			uint16_t unwrapId;
			if (!propStream.read<uint16_t>(unwrapId)) {
				return ATTR_READ_ERROR;
			}

			setIntAttr(ITEM_ATTRIBUTE_UNWRAPID, unwrapId);
			break;
		}
...
void Item::serializeAttr(PropWriteStream& propWriteStream) const
{
	...
	if (hasAttribute(ITEM_ATTRIBUTE_UNWRAPID)) {
		propWriteStream.write<uint8_t>(ATTR_UNWRAPID);
		propWriteStream.write<uint16_t>(getIntAttr(ITEM_ATTRIBUTE_UNWRAPID));
	}
