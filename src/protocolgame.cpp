...
void ProtocolGame::parseTooltip(NetworkMessage& msg) {
    switch (msg.getByte()) {
        case 0: { // Virtual item
            const uint16_t spriteId = msg.get<uint16_t>();
            const uint16_t count    = msg.get<uint16_t>();

            addGameTask(
                static_cast<void (Game::*)(uint32_t, uint16_t, uint16_t)>(&Game::playerSendTooltip),
                player->getID(), spriteId, count
            );
            break;
        }
        case 1: { // Real inventory item
            const Position pos = msg.getPosition();
            const uint16_t spriteId = msg.get<uint16_t>();
            const uint8_t  stackpos = msg.getByte();

            addGameTask(
                static_cast<void (Game::*)(uint32_t, const Position, uint16_t, uint8_t)>(&Game::playerSendTooltip),
                player->getID(), pos, spriteId, stackpos
            );
            break;
        }
        default:
