...
void Game::addMoney(Cylinder* cylinder, uint64_t money, uint32_t flags /*= 0*/)
{
	...
	uint64_t demoniacCoins = money / 100000000;
	money -= demoniacCoins * 100000000;
	while (demoniacCoins > 0) {
		const uint16_t count = std::min<uint64_t>(100, demoniacCoins);
		Item* remaindItem = Item::CreateItem(ITEM_DEMONIAC_COIN, count);
	...
	uint32_t amberCoins = money / 1000000;
	money -= amberCoins * 1000000;
	while (amberCoins > 0) {
		const uint16_t count = std::min<uint32_t>(100, amberCoins);
		Item* remaindItem = Item::CreateItem(ITEM_AMBER_COIN, count);
		ReturnValue ret = internalAddItem(cylinder, remaindItem, INDEX_WHEREEVER, flags);
		if (ret != RETURNVALUE_NOERROR) {
			internalAddItem(cylinder->getTile(), remaindItem, INDEX_WHEREEVER, FLAG_NOLIMIT);
		}
...
slots_t getSlotType(const ItemType& it)
{
	...
	if (it.weaponType != WeaponType_t::WEAPON_SHIELD) {
		...
		} else if (slotPosition & SLOTP_DECK) {
			slot = CONST_SLOT_DECK;
		} else if (slotPosition & SLOTP_BADGE) {
			slot = CONST_SLOT_BADGE;
		} else if (slotPosition & SLOTP_SHIP) {
			slot = CONST_SLOT_SHIP;
...
void Game::playerWrapItem(uint32_t playerId, const Position& position, uint8_t stackPos, const uint16_t spriteId)
{
	...
	uint16_t wrapId = it.wrapId;
	uint16_t unwrapId = it.unwrapId;
	if (item->hasAttribute(ITEM_ATTRIBUTE_UNWRAPID)) {
		unwrapId = item->getIntAttr(ITEM_ATTRIBUTE_UNWRAPID);
	}

	if (!item || item->getClientID() != spriteId || (wrapId == 0 && unwrapId == 0) || item->hasAttribute(ITEM_ATTRIBUTE_UNIQUEID)) {
		player->sendCancelMessage(RETURNVALUE_NOTPOSSIBLE);
		return;
	}
...
bool Game::combatChangeHealth(Creature* attacker, Creature* target, CombatDamage& damage)
{
	...
	if (damage.primary.value > 0) {
		...
		if (attackerMonster && attackerMonster->getLevel() > 0) {
            float bonusDmg = g_config.getFloat(ConfigManager::MLVL_BONUSDMG) * attackerMonster->getLevel();
			if (bonusDmg != 0.0) {
