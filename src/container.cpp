		...
		uint32_t flags, Creature* actor/* = nullptr*/) const
{
	...
	if (getName() == "alicorn Quiver" && !(item->getSlotPosition() & SLOTP_AMMO)) {
        return RETURNVALUE_NOTENOUGHROOM;
    }

    if (getName() == "jungle Quiver" && !(item->getSlotPosition() & SLOTP_AMMO)) {
        return RETURNVALUE_NOTENOUGHROOM;
    }

    if (getName() == "Aeris Quiver" && !(item->getSlotPosition() & SLOTP_AMMO)) {
        return RETURNVALUE_NOTENOUGHROOM;
    }
    if (getName() == "Bazelgeuse Quiver" && !(item->getSlotPosition() & SLOTP_AMMO)) {
        return RETURNVALUE_NOTENOUGHROOM;
    }
    if (getName() == "Kirin Quiver" && !(item->getSlotPosition() & SLOTP_AMMO)) {
        return RETURNVALUE_NOTENOUGHROOM;
    }
    if (getName() == "naga Quiver" && !(item->getSlotPosition() & SLOTP_AMMO)) {
        return RETURNVALUE_NOTENOUGHROOM;
    }
    if (getName() == "eldritch Quiver" && !(item->getSlotPosition() & SLOTP_AMMO)) {
        return RETURNVALUE_NOTENOUGHROOM;
    }
	if (getName() == "Mercurial Quiver" && !(item->getSlotPosition() & SLOTP_AMMO)) {
        return RETURNVALUE_NOTENOUGHROOM;
    }
	if (getName() == "Moist Quiver" && !(item->getSlotPosition() & SLOTP_AMMO)) {
        return RETURNVALUE_NOTENOUGHROOM;
    }
	if (getName() == "Candy-Coated Quiver" && !(item->getSlotPosition() & SLOTP_AMMO)) {
        return RETURNVALUE_NOTENOUGHROOM;
    }
	if (getName() == "Haloed Quiver" && !(item->getSlotPosition() & SLOTP_AMMO)) {
        return RETURNVALUE_NOTENOUGHROOM;
    }
	if (getName() == "Seraphic Quiver" && !(item->getSlotPosition() & SLOTP_AMMO)) {
        return RETURNVALUE_NOTENOUGHROOM;
    }
	
	// =====================
    // Deck checks (Revamped code to allow Upgrade System on Decks +2 .. +15 and only allow cards on it)
    // =====================
	static auto asLowerCaseString = [](const std::string& in) {
        std::string out = in;
    ...
    std::string containerName = asLowerCaseString(getName());
    if (startsWith(containerName, "bronze deck") ||
        startsWith(containerName, "silver deck") ||
        startsWith(containerName, "golden deck"))
    {
        if (item->getSlotPosition() != 48) {
            return RETURNVALUE_NOTENOUGHROOM;
        }
        static const std::set<uint16_t> allowedDeckItems = {
            27635, 27636, 27637, 27638, 27639
        };
        if ((item->getID() >= 27482 && item->getID() <= 27631) ||
             allowedDeckItems.find(item->getID()) != allowedDeckItems.end())
        {
            return RETURNVALUE_NOERROR;
        }
        return RETURNVALUE_NOTENOUGHROOM;
